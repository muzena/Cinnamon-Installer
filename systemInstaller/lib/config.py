...
#
# Original version from: Rémy Oudompheng <remy@archlinux.org>
...
import pyalpm
import os, glob, argparse, collections, warnings

ABS_PATH = os.path.abspath(__file__)
INSTALLER_PATH = os.path.dirname(os.path.dirname(ABS_PATH)) + '/lib/config/installer.conf'
...
def installer_conf_enumerator(path):
    filestack = []
    current_section = None
    filestack.append(open(path))
    while len(filestack) > 0:
...
class InstallerConfig():
    ...
    def load_from_file(self, filename):
        for section, key, value in installer_conf_enumerator(filename):
            if key in LIST_OPTIONS:
                self.options.setdefault(key, []).append(value)
            else:
                self.options[key] = value
    ...
    def reload(self):
        self.options["EnableAUR"]  = False
        self.options["RemoveUnrequiredDeps"] = False
        self.load_from_file(INSTALLER_PATH)

    def __str__(self):
        return("CinnamonInstallerConfig(options={})".format(self.options))

pacman_conf = PacmanConfig(conf = PACMAN_PATH)
